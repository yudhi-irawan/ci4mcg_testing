...
class Sex_model extends Model
{
	protected $table = 'sex';
	protected $primaryKey = 'sex_id';
	protected $useTimestamps = true;
	protected $allowedFields = ['sex_desc'];
	protected $SQL = 'select * from sex_one_view';


    public function getSexById($id = false)
    {
        if ($id == false) {
			$sql1="select * from ( ".$this->SQL." ) as xxx";
			return $this->db->query($sql1)->getResultArray();
        }
        return $this->table('sex')
        	->where('sex_id', $id)
        	->get()
        	->getRowArray();
    }

    public function getSexForLookUp()
	{
	...
	{
		$sql_add = " CALL Sex_one_add";
		$sql_add.= " (";
		$sql_add.= "'99999'";
		$sql_add.= ",'".$arrdata['sex_desc']."'";
		$sql_add.= ")";
	...
	{
		$sql_delete = " CALL Sex_one_delete";
		$sql_delete.= " (";
		$sql_delete.= "'.$id.'";
		$sql_delete.= ")";
